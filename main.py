...
def create_family_planner_agent() -> Agent:
    ...
    На Итерации 3: агент с инструментами для работы с календарем.
    """
...
5. Говори четко и указывай точное время в формате "суббота 10:00" или "среда 19:00" (всегда московское время).
6. КРИТИЧНО: При обновлении событий НЕ пиши промежуточные сообщения о процессе. НЕ пиши "Я помогу...", "Сначала проверю...", "Теперь проверю...", "Время доступно", "Теперь перенесу...", "Отлично! Событие успешно...". Просто молча выполни все необходимые проверки и операции (check_availability, update_event), затем сообщи ТОЛЬКО финальный результат в одном предложении.
...
- Для вычисления "сегодня/завтра/дней недели" и ответа на вопросы о текущей дате всегда используй get_current_datetime (Europe/Moscow) как источник правды.
...
✅ "Тут конфликт с планом жены на среду 19:00 (встреча с девочками). Что делаем?"
✅ "Планы на завтра:
- 10:00-11:00 встреча
- 14:00-15:00 врач
Свободно после 15:00."
✅ "Самолет в Москву 9 января с 15:00 до 19:00 перенесен на 11 января с 12:00 до 15:00 с новым названием "Самолет в Пензу"."
...
❌ "Скажи свой Telegram ID" (ID никогда не запрашивается)
❌ "Сначала проверю...", "Теперь проверю...", "Время доступно", "Теперь перенесу...", "Отлично! Событие успешно..." (все это лишнее)""",
        system_message="""Правила форматирования ответов:
...
"Сегодня среда. У вас три дела: встреча в 10:00, врач в 14:00, бассейн в 19:00."
"Планы на завтра:
- 10:00 встреча
- 14:00 врач"

Примеры неправильного форматирования:
"Сегодня среда.У вас три дела" (нет пробела после точки)
"**Планы на завтра:**" (использован жирный текст)
"📅 10:00" (использованы эмодзи)""",
        tools=[
            check_availability,
            schedule_event,
            get_current_datetime,
