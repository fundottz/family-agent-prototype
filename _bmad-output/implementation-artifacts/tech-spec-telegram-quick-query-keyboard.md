...
- [ ] Task 1: Добавить импорты для клавиатуры
...
- [ ] Task 2: Создать функцию для генерации клавиатуры
  - File: `telegram_bot.py`
...
- [ ] Task 3: Создать команду /keyboard для показа клавиатуры
  - File: `telegram_bot.py`
...
- [ ] Task 4: Обновить функцию start_command для отображения клавиатуры
  - File: `telegram_bot.py`
...
- [ ] Task 5: Обновить handle_message для сохранения клавиатуры после ответов
  - File: `telegram_bot.py`
...
- [ ] Task 6: Добавить тест для создания клавиатуры
...
- [ ] Task 7: Добавить тест для команды /keyboard
  - File: `tests/test_telegram_bot.py`
...
- [ ] Task 8: Добавить тест для отображения клавиатуры в /start
  - File: `tests/test_telegram_bot.py`
...
- [ ] Task 9: Добавить тест для обработки нажатий кнопок
  - File: `tests/test_telegram_bot.py`
...
- [ ] AC 1: Given пользователь отправляет команду `/keyboard`, when бот обрабатывает команду, then внизу экрана отображается клавиатура с 4 кнопками: "Планы на сегодня", "Планы на завтра", "Планы на неделю", "Что на выходных"

- [ ] AC 2: Given пользователь отправляет команду `/start`, when бот отправляет приветственное сообщение, then внизу экрана также отображается клавиатура с 4 кнопками (для удобства)

- [ ] AC 3: Given клавиатура отображается в чате, when пользователь нажимает на кнопку "Планы на сегодня", then текст "Планы на сегодня" отправляется как обычное сообщение и обрабатывается ботом через существующий обработчик `handle_message`

- [ ] AC 4: Given клавиатура отображается в чате, when пользователь нажимает на любую из кнопок ("Планы на завтра", "Планы на неделю", "Что на выходных"), then соответствующий текст отправляется и обрабатывается корректно

- [ ] AC 5: Given пользователь нажал на кнопку клавиатуры, when бот обрабатывает запрос и отправляет ответ, then клавиатура остается видимой после ответа (через `reply_markup` в ответе)

- [ ] AC 6: Given клавиатура отображается в чате, when пользователь отправляет обычное текстовое сообщение (не через кнопку), then сообщение обрабатывается как обычно, клавиатура остается видимой (если была показана ранее)

- [ ] AC 7: Given клавиатура создана функцией `create_quick_query_keyboard()`, when проверяется структура клавиатуры, then она содержит ровно 4 кнопки с правильными текстами, организованными в 2 ряда по 2 кнопки для оптимального отображения на iOS
